...
function DashboardCard({header,icon,content,color}){
    return(
        <div className={`w-80 h-25 md:w-60 relative flex rounded-sm border border-gray-300 md:h-30 shadow-sm ${color}`}>
            <div className={`w-1.5 ${color} h-full absolute left-0 top-0 rounded-l-xl`}></div>
        <div className="flex flex-col w-full pl-4 rounded-sm  h-full bg-white">
